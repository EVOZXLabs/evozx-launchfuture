...
import { NETWORK, STORAGE_KEYS } from "./config.js";
...
export const isConnected = () => account !== null;
...
export async function checkNetwork() {
  ...
  return Number(chainId) === NETWORK.chainId;
}
...
export async function connectWallet() {
  ...
  try {
    if (!(await checkNetwork())) {
      const switched = await switchToEVOZ();
    ...
    localStorage.setItem(STORAGE_KEYS.wallet, "true");
    updateWalletButtons();
...
export async function restoreConnection() {
  if (localStorage.getItem(STORAGE_KEYS.wallet) !== "true") {
    updateWalletButtons();
    return;
  }
...
export function updateWalletButtons() {
  document.querySelectorAll("#connectBtn").forEach((button) => {
    ...
    if (account) {
      button.textContent = shortAddress(account);
